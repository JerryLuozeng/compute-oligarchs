...
import { Activity, Cpu, Database, MapPinned } from "lucide-react";
...
function TileMetric({ icon, label, value }: { icon: React.ReactNode; label: string; value: number }) {
  return (
    ...
    <section className="map-section" aria-label="地块地图">
      <div className="map-section__heading">
        ...
        <div className="map-legend" aria-label="势力图例">
          ...
          <span><i className="legend-dot legend-dot--commons" />公共/废土</span>
        </div>
      </div>

      <div className="map-layout">
        <div className="tile-grid" role="grid" aria-label="六个世界地块">
          {tiles.map((tile, index) => {
            ...
            return (
              ...
              >
                <span className="map-tile__index">0{index + 1}</span>
        ...
        <aside className={`tile-detail ${controllerTone[selectedTile.controllingFaction]}`} aria-live="polite">
          ...
          <p className="tile-detail__controller">{controllerLabel[selectedTile.controllingFaction]} · {selectedTile.collectionMode.replaceAll("_", " ")}</p>
          ...
          <div className="tile-detail__stability">
            ...
            <span className="tile-detail__stability-track"><i style={{ width: `${Math.min(100, selectedTile.stability)}%` }} /></span>
          </div>
        </aside>
      </div>
    </section>
